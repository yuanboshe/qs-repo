...
- `registry.yaml` 和 `.qs/index.json` 用于验证 `qs search` 的远程 registry 和 index-first 读取。
...
- 隐藏目录、隐藏文件、目录内 `recipe.yaml`、目录内 `config.yaml` 和 `.qs/index.json` 不应被 `qs inspect` 当成 template。
